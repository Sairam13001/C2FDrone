...
class feature_enhancement(nn.Module):
	def __init__(self, channel):
		...
		self.conv1 = BasicConv2d(4*channel, channel, 1, padding=0)
		self.conv2 = BasicConv2d(2*channel, channel, 1, padding=0)
		self.conv3 = BasicConv2d(channel, channel, 1, padding=0)

		self.conv_upsample1 = BasicConv2d(channel, channel, 3, padding=1)
		self.conv_upsample2 = BasicConv2d(channel, channel, 3, padding=1)
		self.conv_upsample3 = BasicConv2d(channel, channel, 3, padding=1)
		self.conv_upsample4 = BasicConv2d(2*channel, 2*channel, 3, padding=1)
	...
	def forward(self, inp1, inp2, inp3):
		...
		x2_2 = torch.cat((x2_1, self.conv_upsample3(self.upsample(x1_1))), 1)
		x2_2 = self.conv_concat2(x2_2)

		x3_2 = torch.cat((x3_1, self.conv_upsample4(self.upsample(x2_2))), 1)
...
class SwinTransformer(nn.Module):
	...
	def __init__(self,
		...
		self.feature_enhance = feature_enhancement(channel=256)
	...
	def forward(self, x):
		...
		enhanced_features = self.feature_enhance(rets['2'], rets['1'], rets['0']) 
		...
		return rets_agg, enhanced_features
...
class BackboneBase(nn.Module):
	...
	def forward(self, tensor_list: NestedTensor):
		xs, enhanced_features = self.body(tensor_list.tensors)
...
class Joiner(nn.Sequential):
	...
	def forward(self, samples: NestedTensor):
		xs, enhanced_features = self[0](samples) 
		...
		return out, enhanced_features, pos
