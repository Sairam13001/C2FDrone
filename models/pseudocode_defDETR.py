def compute_detr_inputs(self, features, samples, pos):
        '''
        Objective: features are transformed channel-wise via linear layers
...
class transformer(srcs, masks, pos, query_embeds):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            # level_embed?
            # Paper: "To identify which feature level each query pixel lies in, we add a SCALE-LEVEL EMBEDDING,
            # to the feature representation, in addition to the positional embedding. " (Pg 6)
