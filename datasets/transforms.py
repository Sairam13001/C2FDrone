...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, target, p):
...
class RandomResize(object):
    ...
    def __call__(self, img, target=None, p=None):
        size = self.sizes
...
class ToTensor(object):
    def __call__(self, clip, target, p):
        img = []
        for im in clip:
...
class Normalize(object):
    ...
    def __call__(self, clip, target=None, p=None):
        image = []
        for im in clip:
...
class Compose(object):
    ...
    def __call__(self, image, target, p):
        for t in self.transforms:
            image, target = t(image, target, p)
        return image, target
...
class ColorJitterTransform(object):
    def __init__(self, p=None, brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5):
        self.p = p
    ...
    def __call__(self, clip, target, p):
        if random.random() < 0.5:
            image = []
            for im in clip:
                image.append(self.transform(im))
            return image, target
        return clip, target
