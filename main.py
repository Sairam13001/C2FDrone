...
def get_args_parser():
	...
	# * Backbone
	parser.add_argument('--backbone', default='swin_b_p4w7', type=str,
	...
	parser.add_argument('--set_cost_class', default=2, type=float,
						help="Class coefficient in the matching cost")  #2, 5, 2 for class, box, giou in default
	...
	parser.add_argument('--feat_aggr_attn_loss_coef', default=2, type=float)
	parser.add_argument('--instance_aware_feat_aggr_attn_loss_coef', default=1, type=float)
	parser.add_argument('--dab_detr_decoder_query_loss_coef', default=1, type=float)
	parser.add_argument('--dab_detr_decoder_query_width_and_height_loss_coef', default=1, type=float)

	# dataset parameters
	parser.add_argument('--dataset_file', default='vid_single')
	parser.add_argument('--coco_path', default='./data/coco', type=str)
	parser.add_argument('--vid_path', default='data/FL_Drones_full_train/', type=str)
...
def main(args):
	...
	from engine import evaluate, train_one_epoch, validate  #, evaluate_fps
	...
	if args.distributed:
		if args.cache_mode:
			sampler_train = samplers.NodeDistributedSampler(dataset_train, shuffle = True)
			sampler_val = samplers.NodeDistributedSampler(dataset_val, shuffle=True)
		else:
	...
	# lr_scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=3, factor=0.1, verbose=True)

	if args.distributed:
		#print('DISTRIBUTED ____________')
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		lr_scheduler.step()
		# lr_scheduler.step(valdn_loss)
		print('args.output_dir', args.output_dir)
		if args.output_dir:
			checkpoint_paths = [output_dir / 'checkpoint.pth']
			# extra checkpoint before LR drop and every 5 epochs
			# if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 1 == 0:
			if (epoch == 11) or (epoch == 14):
